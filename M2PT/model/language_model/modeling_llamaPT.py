...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, PT_len=100,VIT_PT_LEN=999, **kwargs):
    ...
    def forward(
        ...
        if self.prompts_token_len > 0:
            if past_key_values is None:
                if self.index>2:
                    print(f".Sum:{torch.sum(self.debug_weights - self.prompts[-5])}")
                    print(f"{self.prompts[-5].requires_grad}")
        ...
        for idx, decoder_layer in enumerate(self.layers):
            if self.prompts_token_len > 0 and past_key_values is None:
                hidden_states[:, :self.prompts_token_len, :] = self.prompts[idx].repeat(batch_size, 1, 1).to(inputs_embeds.device).to(inputs_embeds.dtype)
